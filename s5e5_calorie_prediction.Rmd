...
##### diagnose
```{r}
mod_list$base |>augment(newdata = tr) |> rename(.pred= .fitted) |>mutate(.residual=log1p(calories)-log1p(.pred)) -> tmp
tmp|>ggplot(aes(x=age, y=.residual)) +geom_smooth()
#after heart_rate >125 has high residuals
...
form_base=calories ~ s(age,k=15) +s(duration,k=15) + s(heart_rate,k=10) + s(body_temp,k=10) + s(bmi,k=10) + factor(sex_male)
